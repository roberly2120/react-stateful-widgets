...
const [featuredProgrammer, setFeaturedProgrammer] = useState(null)
  ...
  const getNameOfFeatured = () => {
    ...
    for (let i = 0; i < programmerList.length; i++) {
      if (featuredProgrammer == programmerList[i].id) {
        return programmerList[i].name;
      }
    }
    // to use the find method.
  // const foundDev = programmers.find(dev => dev.is === featuredProgrammer);
